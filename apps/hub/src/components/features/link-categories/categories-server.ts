...
import type { CategoryId } from "@/components/features/link-categories/categories-types"; // 从 categories-types 导入
...
export async function generateCategoriesFromFiles(): Promise<LinkCategory[]> {
  ...
  try {
    ...
    const linksFilePath = path.join(process.cwd(), "src/components/features/links/links-data.json");
