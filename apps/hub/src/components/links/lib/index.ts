...
const categoryStructure = {
  ...
  media: {
    ...
    children: {
      audio: {
        name: "音频处理",
        file: () => safeImport(() => import("@/content/media/audio.json")),
      },
      images: {
        name: "图像处理",
        file: () => safeImport(() => import("@/content/media/images.json")),
      },
      video: {
        name: "视频处理",
        file: () => safeImport(() => import("@/content/media/video.json")),
      },
    },
  },
  office: {
...
async function loadAllLinksData(_cacheKey = ""): Promise<LinksItem[]> {
  ...
  if (storageCachedData && storageCachedData.length > 0) {
    console.log(
      "使用localStorage缓存的数据，条目数:",
      storageCachedData.length,
    );
