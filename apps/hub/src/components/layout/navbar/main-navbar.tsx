...
import { GitHubButton, TravelButton } from "@/components/features/buttons";
import { ThemeToggle } from "@/components/theme";
import { Logo } from "./logo";
import { NavListMenu } from "./nav-menu";
import { MobileMenu } from "./mobile-menu";
import { useMobileSidebarWithCallback } from "./mobile-sidebar-provider";

// 动态导入搜索按钮组件
const SearchButton = dynamic(
  () => import("@/components/features/search/search-button").then((mod) => mod.SearchButton),
  { ssr: false }
);

export const MainNavbar = ({ className = "" }: { className?: string }) => {
  const [isScrolled, setIsScrolled] = useState(false);
  const { getSidebarContentWithCallback } = useMobileSidebarWithCallback();
    ...
    >
      <div className="container mx-auto flex h-full items-center justify-between px-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* 移动端菜单 */}
          <MobileMenu
            sidebarContentWithCallback={(onLinkClick) =>
              getSidebarContentWithCallback(onLinkClick, "", () => {
                // Category change handler - could be enhanced to sync with main page
              })
            }
          />
        </div>
