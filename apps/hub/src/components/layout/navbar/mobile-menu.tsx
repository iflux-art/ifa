...
import { NAV_ITEMS, NAV_PATHS, NAV_DESCRIPTIONS } from "./nav-config";
...
export const MobileMenu = ({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      // 菜单打开时，将焦点设置到第一个导航项
      setTimeout(() => {
        const firstNavItem = document.querySelector('[tabindex="0"]') as HTMLElement;
        if (firstNavItem) {
          firstNavItem.focus();
        }
      }, 100);
          ...
          <div className="fixed inset-0 overflow-y-auto bg-background transition-transform duration-300 ease-out">
            ...
            <div className="p-6">
              <p id={menuDescriptionId} className="sr-only">
                使用方向键或Tab键导航，按Enter键选择，按Escape键关闭菜单
              </p>

              {/* 侧边栏内容区域 - 12列网格布局，每行1张卡片 */}
              {(sidebarContent || sidebarContentWithCallback) && (
                <div className="mb-8">
                  <h3 className="mb-4 font-medium text-base">分类导航</h3>
                  <div className="grid grid-cols-12 gap-4">
                    <div className="col-span-12">
                      {sidebarContentWithCallback
                        ? sidebarContentWithCallback(closeMenu)
                        : sidebarContent}
                    </div>
                  </div>
                </div>
              ...
              {/* 导航卡片网格 - 12列布局，每行2张卡片 */}
              <nav aria-label="主导航">
                <h3 className="mb-4 font-medium text-base">导航</h3>
                <div className="grid grid-cols-12 gap-4">
                  {NAV_ITEMS.map((item: NavConfigItem) => {
                    ...
                    const renderIcon = () => {
                      if (!item.icon) {
                        return <div className="h-6 w-6 rounded bg-current" />;
                      }
                    ...
                    const cardContent = (
                      <div className="group col-span-6">
                        <div className="relative overflow-hidden rounded-lg border bg-card p-4 shadow-sm transition-all duration-200 ease-out focus-within:ring-2 focus-within:ring-primary focus-within:ring-offset-2 hover:scale-[1.02] hover:border-primary/20 hover:shadow-md active:scale-[0.98]">
