 ...
 */
function getResponsiveClasses(mobile: boolean, tablet: boolean, desktop: boolean): string {
  // 使用位运算生成索引：移动端(bit 2) + 平板(bit 1) + PC(bit 0)
  const index = (mobile ? 0b100 : 0) | (tablet ? 0b010 : 0) | (desktop ? 0b001 : 0);
  return RESPONSIVE_CLASS_MAP[index as keyof typeof RESPONSIVE_CLASS_MAP] || "block";
}
