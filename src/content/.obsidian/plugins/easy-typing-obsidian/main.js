...
var EasyTypingSettingTab = class extends import_obsidian2.PluginSettingTab {
	...
	display() {
		...
		new import_obsidian2.Setting(containerEl)
			.setName(locale5.settings.backspaceEdit.name)
			.setDesc(locale5.settings.backspaceEdit.desc)
			.addToggle((toggle) => {
				toggle
		...
		new import_obsidian2.Setting(containerEl)
			.setName(locale5.settings.quoteSpace.name)
			.setDesc(locale5.settings.quoteSpace.desc)
			.addToggle((toggle) => {
				toggle
					.setValue(this.plugin.settings.QuoteSpace)
					.onChange(async (value) => {
		...
		new import_obsidian2.Setting(containerEl)
			...
			.addDropdown((dropdown) => {
				dropdown.addOption(
					"enter_twice" /* EnterTwice */,
					locale5.dropdownOptions.enterTwice,
				);
				dropdown.addOption(
					"two_space" /* TwoSpace */,
					locale5.dropdownOptions.twoSpace,
				);
				dropdown.addOption(
					"mix_mode" /* Mix */,
					locale5.dropdownOptions.mixMode,
				);
		...
		new import_obsidian2.Setting(containerEl)
			.setName(locale5.settings.enhanceModA.name)
			.setDesc(locale5.settings.enhanceModA.desc)
			.addToggle((toggle) => {
				toggle
...
function selectCodeBlockInPos(view, selection) {
	...
	for (let i = 0; i < codeBlockInfos.length; i++) {
		if (
			...
			let code_line_start = view.state.doc.lineAt(
				codeBlockInfos[i].code_start_pos,
			);
			let isCodeSelected =
				selection.anchor == code_line_start.from &&
				selection.head == codeBlockInfos[i].code_end_pos;
			let isCodeBlockSelected =
				selection.anchor == codeBlockInfos[i].start_pos &&
...
function getQuoteInfoInPos(state, pos) {
	...
	if (match) {
		...
		cur_start_pos =
			cur_line.from +
			(match_callout ? match_callout[0].length : match[0].length);
...
var EasyTypingPlugin = class extends import_obsidian3.Plugin {
	constructor() {
		...
		this.handleEnter = (view) => {
			...
			if (
				this.settings.BetterCodeEdit &&
				codeBlockInfo &&
				codeBlockInfo.code_start_pos !==
					doc.lineAt(codeBlockInfo.start_pos).to &&
				pos >= codeBlockInfo.code_start_pos &&
				pos <= codeBlockInfo.code_end_pos
			) {
				let line_indent_str =
					((_a = line.text.match(/^\s*/)) == null ? void 0 : _a[0]) || "";
				view.dispatch({
					changes: { from: pos, to: pos, insert: "\n" + line_indent_str },
					selection: {
						anchor: pos + line_indent_str.length + 1,
						head: pos + line_indent_str.length + 1,
					},
			...
			if (getPosLineType2(state, pos) == "quote" /* quote */) {
				...
				let quote_indent_str =
					(quote_match == null ? void 0 : quote_match[1]) || "";
				let quote_level =
					(quote_match == null ? void 0 : quote_match[2].length) || 0;
				let quote_content = line.text.slice(quote_match[0].length);
				if (quote_content.trim() == "") return false;
				else {
					let space_str2 = "  ";
					if (quote_content.endsWith("  ")) space_str2 = "";
			...
			if (
				this.settings.StrictLineMode == "two_space" /* TwoSpace */ &&
				getPosLineType2(state, pos) == "text" /* text */
			) {
				let inserted_str = space_str + "\n";
				view.dispatch({
					changes: { from: pos, to: pos, insert: inserted_str },
					selection: {
						anchor: pos + inserted_str.length,
						head: pos + inserted_str.length,
					},
					userEvent: "EasyTyping.handleEnter",
				});
				return true;
			...
			if (
				getPosLineType2(state, pos) == "text" /* text */ ||
				(codeBlockInfo &&
					pos == codeBlockInfo.end_pos &&
					codeBlockInfo.indent == 0)
			) {
				view.dispatch({
					changes: {
						from: pos,
		...
		this.handleModA = (view) => {
			...
			if (
				this.settings.EnhanceModA &&
				line_type == "text" /* text */ &&
				!is_in_code_block
			) {
				let [block_start, block_end] = this.getBlockLinesInPos(
					view.state,
					selection.head,
				);
				if (
					selection.anchor <= view.state.doc.line(block_start).from &&
					selection.head >= view.state.doc.line(block_end).to
				) {
					return false;
				}
			...
			if (this.settings.EnhanceModA && quote_info) {
				if (
					selection.anchor == quote_info.start_pos &&
					selection.head == quote_info.end_pos
				) {
					return false;
				} else if (
					selection.anchor == quote_info.cur_start_pos &&
					selection.head == quote_info.cur_end_pos
				) {
					view.dispatch({
						selection: {
							anchor: quote_info.start_pos,
							head: quote_info.end_pos,
						},
						userEvent: "EasyTyping.handleModA",
					});
					return true;
					...
					view.dispatch({
						selection: {
							anchor: quote_info.cur_start_pos,
							head: quote_info.cur_end_pos,
						},
						userEvent: "EasyTyping.handleModA",
					});
					return true;
	...
	async onload() {
		...
		let ExtraBasicConvRuleStringList = [
			["r/(?<=^|\\n)(\\s*>*) ?[>\u300B]/|", "[[0]]> |"],
		];
		let QuoteSpaceRuleStringList = [
			["r/(?<=^|\\n)(\\s*>+)([^ >\u300B]+)/|", "[[0]] [[1]]|"],
		];
		this.ExtraBasicConvRules = ruleStringList2RuleList(
			ExtraBasicConvRuleStringList,
		);
	...
	getBlockLinesInPos(state, pos) {
		...
		for (let i = line.number - 1; i >= 1; i--) {
			...
			if (
				getPosLineType2(state, line2.from) == "text" /* text */ &&
				line2.text !== "" &&
				!reg_headings.test(line2.text)
			) {
				block_start = i;
				continue;
			}
	...
	toggleCodeBlockLineComment(from, to, text, commentSymbol, cursor_pos) {
		...
		if (typeof commentSymbol === "string") {
			...
			if (trimmedText.startsWith(commentSymbol)) {
				...
				return {
					...
					to:
						from +
						commentIndex +
						commentSymbol.length +
						(trimmedText.startsWith(commentSymbol + " ") ? 1 : 0),
					insert: "",
				};
			} else {
			...
			if (
				trimmedText.startsWith(commentSymbol.start) &&
				trimmedText.endsWith(commentSymbol.end)
			) {
				const commentStartIndex = text.indexOf(commentSymbol.start);
				return {
					from: from + commentStartIndex,
					to,
					insert: trimmedText.slice(
						commentSymbol.start.length + 1,
						-commentSymbol.end.length - 1,
					),
				};
			} else {
	...
	handleShiftEnter(view) {
		...
		if (taskListMatch) {
			...
			view.dispatch({
				...
				selection: {
					anchor: selection.anchor + inserted.length,
					head: selection.anchor + inserted.length,
				},
	...
	handleBackspace(view) {
		...
		if ((listMatchEmpty || quoteMatchEmpty) && selection.anchor == line.to) {
			...
			if (quoteMatchEmpty) {
				...
				if (quoteLevel > 1) {
					if (line.number > 1) {
						...
						if (
							...
							let temp_line =
								quote_indent_str + ">".repeat(quoteLevel - 1) + " ";
							...
							changes = [
								{ from: line.from, to: line.to, insert: newQuotePrefix },
							];
				...
				let expectedNextNumber = currentListMatch
					? parseInt(currentListMatch[1], 10) + 1
					: null;
				while (nextLineNumber <= doc.lines && expectedNextNumber !== null) {
					...
					if (nextListMatch) {
						...
						if (nextListNumber === expectedNextNumber) {
							const newNextLineContent = nextLineContent.replace(
								/^\s*\d+\.\s/,
								`${nextIndent}${nextListNumber - 1}. `,
							);
							changes.push({
								from: nextLine.from,
								to: nextLine.to,
								insert: newNextLineContent,
							});
							expectedNextNumber++;
						} else {
							break;
	...
	getCommandNameMap() {
		...
		let command_name_map_ru = /* @__PURE__ */ new Map([
			...
			[
				"goto_new_line_after_cur_line",
				"\u041F\u0435\u0440\u0435\u0439\u0442\u0438 \u043A \u043D\u043E\u0432\u043E\u0439 \u0441\u0442\u0440\u043E\u043A\u0435 \u043F\u043E\u0441\u043B\u0435 \u0442\u0435\u043A\u0443\u0449\u0435\u0439",
			],
			[
				"select_block",
				"\u0412\u044B\u0431\u0440\u0430\u0442\u044C \u0442\u0435\u043A\u0443\u0449\u0438\u0439 \u0442\u0435\u043A\u0441\u0442\u043E\u0432\u044B\u0439 \u0431\u043B\u043E\u043A",
			],
		]);
