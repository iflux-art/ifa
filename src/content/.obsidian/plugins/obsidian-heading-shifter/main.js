...
var HeadingShifterSettingTab = class extends import_obsidian.PluginSettingTab {
	...
	display() {
		...
		new import_obsidian.Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle.setValue(this.plugin.settings.overrideTab).onChange((value) =>
					__async(this, null, function* () {
		...
		new import_obsidian.Setting(containerEl)
			...
			.addToggle((toggle) => {
				...
				return toggle
					.setValue(
						(_b =
							(_a = this.plugin.settings.styleToRemove) == null
								? void 0
								: _a.beginning) == null
							? void 0
							: _b.ul,
		...
		new import_obsidian.Setting(containerEl)
			...
			.addTextArea((str) => {
				...
				str
					.setValue(
						(_b =
							(_a = this.plugin.settings.styleToRemove.beginning) == null
								? void 0
								: _a.userDefined) == null
							? void 0
							: _b.join("\n"),
...
var getHeadingLines = (editor, from, to, options) => {
	...
	for (let line = Math.min(from, to); line <= Math.max(from, to); line++) {
		...
		if (
			(options == null ? void 0 : options.includesNoHeadingsLine) ||
			heading > 0
		) {
...
var applyHeading = (chunk, headingSize, settings) => {
	const extractRegExp = (settingObj, regExpObj) => {
		return Object.entries(settingObj != null ? settingObj : {}).flatMap(
			([k, v]) => {
				if (Array.isArray(v)) {
					return v;
				}
	...
	if (!checkHeading(chunk)) {
		removed = (settings == null ? void 0 : settings.styleToRemove)
...
var ApplyHeading = class {
	constructor(settings, headingSize) {
		...
		__publicField(this, "editorCallback", (editor) => {
			...
			editor.transaction({
				changes: composeLineChanges(editor, lines, (chunk) =>
	...
	constructor(settings) {
		...
		__publicField(this, "editorCallback", (editor) => {
			...
			if (
				...
				new import_obsidian4.Notice(
					`Cannot Decrease (contains less than Heading${Number(this.settings.limitHeadingFrom)})`,
				);
				return true;
			}
			const isOneLine =
...
var InsertHeadingAtCurrentLevel = class {
	constructor(settings) {
		...
		__publicField(this, "editorCallback", (editor) => {
			...
			const headingLevel =
				lastHeadingLine != void 0
					? checkHeading(editor.getLine(lastHeadingLine))
					: 0;
			editor.transaction({
				changes: composeLineChanges(editor, [cursorLine], (chunk) =>
